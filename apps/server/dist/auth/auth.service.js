...
let AuthService = class AuthService {
    ...
    async registerUser(registerUserDTO) {
        ...
        if (await this.userService.findByCredentials({
            email,
            username,
        })) {
    ...
    async loginSocial(profile, provider) {
        const user = await this.userService.findOneBySocialId(provider, profile.id);
        if (!user) {
            throw new common_1.ConflictException('User is not registered');
        }
        return user;
    }
    async registerSocial(profile, provider) {
